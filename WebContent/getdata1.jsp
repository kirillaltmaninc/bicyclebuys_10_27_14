...
if(b==false)
  ...
  <table>
		...
		<tr>
		  <td> 
		  <input type="radio"   style=" margin: 0 10px 0 0; float: left;"   id="abccc" value="<%=rs.getString("ShippingCost")%>,<%=rs.getString("ShippingMethod")%>" name="shipcost" onchange="getcalculate1(this.value,<%=total%>,<%=session.getAttribute("couponprice")%>);"></div><div class="radio-bton"><%=rs.getString("ShippingMethod") %> &nbsp;&nbsp;$  <%=rs.getString("ShippingCost") %></div>
		 </td> 
		</tr>
...
<table>
		...
		<tr>
		  <td> 
		  <input type="radio"   style=" margin: 0 10px 0 0; float: left;"   id="abccc" value="<%=ov1p+ov2p+ov3p%>,<%=rs3.getString("ShippingMethod")%>" name="shipcost" onchange="getcalculate1(this.value,<%=total%>,<%=session.getAttribute("couponprice")%>);"></div><div class="radio-bton"><%=rs3.getString("ShippingMethod") %> &nbsp;&nbsp;$  <%=ov1p+ov2p+ov3p %></div>
		 </td> 
		</tr>
